...
def _build_ytt_api():
    """If Webshare proxy credentials are set (as env vars), route transcript
    requests through them — YouTube blocks datacenter IPs like Render's, but
    allows residential/proxy IPs. Falls back to direct (no proxy) otherwise."""
    ...
    if ws_user and ws_pass:
        try:
            ...
            print(f"[transcript] proxy setup failed, continuing without proxy: {e}")
    return None
